...
const BlogCards = ({blogs, currentPage, selectedCategory, pageSize}) => {
  ...
  return (
    <div className='grid md:grid-cols-3 sm:grid-cols-2 grid-cols-1 gap-8'>
        {
            filteredBlogs.map(blogs => <Link to={`/blogs/${blogs.id}`} key={blogs.id} className='p-5 shadow-lg rounded cursor-pointer'>
                <div>
                ...
                <p className='mb-2 text-gray-600'>< FaUser className='inline-flex items-center mr-2'/>{blogs.author}</p>
